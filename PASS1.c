...
void main()
{
    ...
    if(strcmp(opcode,"START")==0)	///check for opcode start
    {
	sscanf(operand,"%X",&start);
        locctr=start;			///locctr = starting address
        fprintf(fp2,"\t%s\t%s\t%X\n",label,opcode,locctr);
        fscanf(fp1,"%s\t%s\t%s",label,opcode,operand);
    }
    else
    ...
    while(strcmp(opcode,"END")!=0 && error==0)	///check if opcode is end
    {
        if(strcmp(label,"#")!=0)		///check for comment if any
        {
